...
class RegionProposalNetwork(object):
    ...
    def build_rpn(self):
        ...
        with tf.variable_scope('cls'):
            self.rpn_cls_score = tf.nn.conv2d(
                feat, self.weights['w_rpn_cls_score'], strides=[
                    1, 1, 1, 1], padding='SAME')

        # bounding-box prediction
        with tf.variable_scope('reg'):
            self.rpn_reg_pred = tf.nn.conv2d(
                feat, self.weights['w_rpn_bbox_pred'], strides=[
                    1, 1, 1, 1], padding='SAME')
    ...
    def rpn_cls_loss(self, rpn_cls_score, rpn_labels):
        ...
        rpn_cls_score = tf.reshape(
            rpn_cls_score, [
                shape[0], 2, shape[3] // 2 * shape[1], shape[2]])
        ...
        # Ignore label=-1 (Neither object nor background: IoU between 0.3 and
        # 0.7)
        rpn_cls_score = tf.reshape(tf.gather(
            rpn_cls_score, tf.where(tf.not_equal(rpn_labels, -1))), [-1, 2])
        rpn_labels = tf.reshape(tf.gather(
            rpn_labels, tf.where(tf.not_equal(rpn_labels, -1))), [-1])
        # Cross entropy error
    ...
    def rpn_bbox_loss(
        ...
        rpn_bbox_reg = 10 * \
            tf.reduce_sum(tf.multiply(tf.rpn_outside_weights, diff_sL1))
        return rpn_bbox_reg
