...
def vgg_16(inputs,
  ...
  with variable_scope.variable_scope(scope, 'vgg_16', [inputs]) as sc:
    ...
    with arg_scope(
      ...
      end_points = utils.convert_collection_to_dict(end_points_collection)
      if spatial_squeeze:
        net = array_ops.squeeze(net, name='conv5/squeezed')
...
def fix_variables(self, sess, pretrained_model):
    ...
    with tf.variable_scope('Fix_VGG16'):
        with tf.device("/cpu:0"):
            ...
            restorer_fc = tf.train.Saver({"fc6/weights": fc6_conv,
                                          "fc7/weights": fc7_conv,
                                          "conv1/conv1_1/weights": conv1_rgb})
            restorer_fc.restore(sess, pretrained_model)

            sess.run(tf.assign(self._variables_to_fix['fc6/weights:0'], tf.reshape(fc6_conv,
                                                                                          self._variables_to_fix['fc6/weights:0'].get_shape())))
            sess.run(tf.assign(self._variables_to_fix['fc7/weights:0'], tf.reshape(fc7_conv,
                                                                                          self._variables_to_fix['fc7/weights:0'].get_shape())))
            sess.run(tf.assign(self._variables_to_fix['conv1/conv1_1/weights:0'],
